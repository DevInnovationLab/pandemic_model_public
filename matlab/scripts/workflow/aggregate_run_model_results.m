function aggregate_run_model_results(sim_results_path)
    % Aggregate per-chunk simulation results into processed output files.
    %
    % Vertically concatenates relative_sums and status_quo_sums across all chunk
    ...
    % Vertcat status_quo_sums across chunks (same layout as relative_sums)
    status_quo_tables = cell(length(chunk_dirs), 1);
    for j = 1:length(chunk_dirs)
        chunk_dir = fullfile(raw_results_path, chunk_dirs(j).name);
        sums_file = fullfile(chunk_dir, 'status_quo_sums.mat');
        if ~exist(sums_file, 'file')
            error('Missing status_quo_sums for aggregation: %s', sums_file);
        end
        status_quo_tables{j} = load(sums_file).baseline_sums;
    end

    all_status_quo_sums = vertcat(status_quo_tables{:});
    save(fullfile(processed_dir, 'status_quo_annual_sums.mat'), 'all_status_quo_sums', '-v7.3');

function plot_dot_whisker(out_dir)
    % PLOT_DOT_WHISKER
    % Plot dot-whisker of net value (mean, 95%% bootstrap CI, raw 2.5--97.5%% range)
    ...
    for k = 1:n
        ...
        boot_ci(k, :) = prctile(bootstat, [2.5, 97.5]);
        perc_range(k, :) = prctile(data, [2.5, 97.5]);
    end
    ...
    % Plot mean with 95% bootstrap confidence intervals using errorbar
    % (This draws a dot for the mean and a whisker for the CI)
    mean_err_low = mean_val - boot_ci(:,1);
    mean_err_high = boot_ci(:,2) - mean_val;
    h_boot = errorbar(ax, mean_val, y_pos, mean_err_low, mean_err_high, ...
        'horizontal', ...
        'ko', ...
        'MarkerFaceColor', 'k', ...
        'MarkerSize', 6.5, ...
        'LineWidth', 1.3, ...
        'CapSize', 14, ...
        'Color', [0.05 0.07 0.13], ...
        'LineStyle', 'none');  % no line connecting scenario means
    ...
    x_lo = min([mean_val; boot_ci(:); perc_range(:)]) - 0.08 * x_range;
    x_hi = max([mean_val; boot_ci(:); perc_range(:)]) + 0.05 * x_range;
    ax.XLim = [x_lo, x_hi];
    ...
    % Legend (note that errorbar combines dot and whisker for mean & CI)
    legend(ax, ...
        [h_raw(1), h_boot], ...
        {'2.5--97.5% sample percentiles', 'Mean (dot) & 95% bootstrap CI (whisker)'}, ...
        'Location', 'best', ...

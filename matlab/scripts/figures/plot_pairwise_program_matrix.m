function plot_pairwise_program_matrix(job_dir)
    ...
    scenarios = scenarios(~strcmp(scenarios, 'status_quo') & ...
    ...
    % Two-line tick labels: TeX \newline in the string + TickLabelInterpreter 'tex'
    % (see MATLAB Central answers/101922, Adam Danz). Literal newline chars in tick
    % labels break across ticks; this keeps native ticks without the LaTeX font.
    ...
    row2 = cell(1, n_inv);
    for k = 1:n_inv
        [row1{k}, row2{k}] = investment_tex_two_lines(investments(k));
    end
    labelArray = [row1; row2];
    tick_block = strtrim(sprintf('%s\\newline%s\n', labelArray{:}));
    tick_cells = cellstr(split(string(tick_block), newline));
    assert(numel(tick_cells) == n_inv, 'pairwise matrix: tick label count mismatch');
    ...
    set(ax, 'XLim', [0.5, n_inv + 0.5], 'YLim', [0.5, n_inv + 0.5]);

    apply_paper_axis_style(ax, spec);
    % apply_paper_axis_style forces ax.FontSize to typography.tick; restore label size.
    % Apply tick strings after styling so TeX markup (\newline, \&) is not replaced by none.
    set(ax, 'YDir', 'reverse', ...
        'XTickLabel', tick_cells, 'YTickLabel', tick_cells, ...
...
function [line1, line2] = investment_tex_two_lines(investment)
% Return two TeX tick lines (use with TickLabelInterpreter 'tex' and \newline between).

    keys = ["advance_capacity", "early_warning", "neglected_pathogen", "universal_flu"];
    line1_opts = {'Advance', 'Early', 'Prototype', 'Universal flu'};
    % Plain text here; escape_tex_tick_label fixes &, %, etc. for TeX ticks.
    line2_opts = {'capacity', 'warning', 'vaccine R&D', 'vaccine R&D'};
    key = string(investment);
    idx = find(keys == key, 1);
    if isempty(idx)
        error("plot_pairwise_program_matrix:UnknownInvestment", "Unknown investment: %s", key);
    end
    line1 = line1_opts{idx};
    line2 = line2_opts{idx};
end
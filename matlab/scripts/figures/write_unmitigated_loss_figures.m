function write_unmitigated_loss_figures(sensitivity_dir, annualized_summary_table, total_summary_table)
    ...
    %
    % Args:
    %   sensitivity_dir (char or string): Program sensitivity directory (contains
    %     sensitivity_config.yaml and scenario folders). Baseline losses are read from
    %     default/ via build_sensitivity_loss_tables.
...
function write_to_latex(summary_data, outpath)
    ...
    if n_data == 5
        ...
        fprintf(fileID, '\\caption{\\textbf{Expected global pandemic deaths and losses in the absence of mitigations (totals over the simulation horizon).} Mortality, economic, learning, and total columns use discounted monetized losses; the last column is the horizon total with productivity growth on the path and no social discount rate. Each cell presents the mean estimate.}\n');
    end
    ...
    if n_data == 5
        ...
        fprintf(fileID, 'Scenario & \\shortstack[c]{Total deaths\\\\(millions)} & \\multicolumn{4}{c}{\\shortstack[c]{Discounted losses \\\\ (trillion \\$)}} & \\shortstack[c]{Horizon total\\\\(growth path,\\\\no social discount)}\\\\\n');
        fprintf(fileID, '\\hline\n');
        fprintf(fileID, ' & & Mortality & Economic & Learning & Total & (trillion \\$) \\\\\n');
    end
    fprintf(fileID, '\\hline\n');
...
% Draw group header text left of axis (no tick marks). Right-aligned with small gap.
    ...
    for k = 1:numel(header_y)
        text(ax, x_pos, header_y(k), header_labels(k), ...
            ...
            'FontName', font_nm, 'FontWeight', 'bold', 'FontSize', header_fs - 1, ...
            'Interpreter', 'tex', 'Color', [0.2 0.2 0.2]);
    end
end

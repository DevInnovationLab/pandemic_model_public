function [out_tbl, info] = simulation_at_pct(job_dir, scenario_name, percentile)
    ...
    %   - Status quo: benefits_vaccine_full - costs_inp_pv_full (absolute).
    ...
    %   scenario_name (char/string): "status_quo" or a scenario name.
    ...
    is_baseline = strcmpi(scenario_name, 'status_quo');

    processed_dir = fullfile(job_dir, 'processed');
    if is_baseline
        S = load(fullfile(processed_dir, 'status_quo_annual_sums.mat'), 'all_status_quo_sums');
        r = S.all_status_quo_sums;
...
function [out_tbl, pan_path, chunk_idx] = load_rows_for_sim(job_dir, scenario_name, sim_num)
    ...
    for i = 1:numel(chunk_dirs)
        ...
        cand_tbl = cand_tbl(~cand_tbl.is_false, :);
        cand_tbl.is_false = [];
        if ~isempty(cand_tbl)
            out_tbl = cand_tbl;
            pan_path = cand_path;

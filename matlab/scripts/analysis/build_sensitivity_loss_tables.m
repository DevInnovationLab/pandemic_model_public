function [annualized_summary_table, total_summary_table] = build_sensitivity_loss_tables(sensitivity_dir)
    ...
    %
    % Loads unmitigated loss outputs for the default baseline and each sensitivity value (from
    ...
    % last column is sim_horizon_total_usd_growth_path (productivity growth on the path, no r).
    %
    % Args:
    ...
    %     TotalHorizonUsdGrowthPath matches sim_horizon_total_usd_growth_path (y on path, no r).
    %
    % Also writes:
    ...
    [baseline_dir, baseline_config] = resolve_sensitivity_reference_run(sensitivity_dir);
    ...
    total_summary_table.Properties.VariableNames = ...
        ...
        'TotalLoss', 'TotalHorizonUsdGrowthPath'};

    [vname, vval] = format_names('default', '', baseline_config, baseline_config);
    mat_file = fullfile(baseline_dir, 'unmitigated_losses.mat');
    S = load(mat_file, 'sim_total_deaths', 'sim_horizon_mortality_usd_social_pv', ...
        'sim_horizon_output_usd_social_pv', 'sim_horizon_learning_usd_social_pv', ...
        'sim_horizon_total_usd_social_pv', 'sim_horizon_total_usd_growth_path');
...
function [annualized_row, total_row] = build_table_rows_from_sim(S, r, periods, variable, value)
    ...
    % Annualized losses use the same annuity factor as estimate_unmitigated_losses (horizon .* ann).
    % Horizon loss columns use social_pv totals; last column is growth_path (y on path, no r).
    ann = annualization_factor(r, periods);
    annualized_row = {...
        variable, value, ...
        {get_mean_and_percentiles(S.sim_total_deaths ./ periods)}, ...
        {get_mean_and_percentiles(S.sim_horizon_mortality_usd_social_pv .* ann)}, ...
        {get_mean_and_percentiles(S.sim_horizon_output_usd_social_pv .* ann)}, ...
        {get_mean_and_percentiles(S.sim_horizon_learning_usd_social_pv .* ann)}, ...
        {get_mean_and_percentiles(S.sim_horizon_total_usd_social_pv .* ann)}, ...
    };
    total_row = {...
        variable, value, ...
        {get_mean_and_percentiles(S.sim_total_deaths)}, ...
        {get_mean_and_percentiles(S.sim_horizon_mortality_usd_social_pv)}, ...
        {get_mean_and_percentiles(S.sim_horizon_output_usd_social_pv)}, ...
        {get_mean_and_percentiles(S.sim_horizon_learning_usd_social_pv)}, ...
        {get_mean_and_percentiles(S.sim_horizon_total_usd_social_pv)}, ...
        {get_mean_and_percentiles(S.sim_horizon_total_usd_growth_path)}, ...
    };
end
...
function [formatted_name, formatted_value] = format_names(var_name, var_value, baseline_config, run_config)
    ...
    switch var_name
        ...
        case "ptrs_pathogen_gamma1"
            ...
            formatted_value = "";
        case "default"
            % Must match order_rows_and_labels / write_to_latex baseline group ("Baseline").
...
function [ref_dir, ref_config] = resolve_sensitivity_reference_run(sensitivity_dir)
    ref_dir = fullfile(sensitivity_dir, 'default');
    ref_config = yaml.loadFile(fullfile(ref_dir, 'run_config.yaml'));
end

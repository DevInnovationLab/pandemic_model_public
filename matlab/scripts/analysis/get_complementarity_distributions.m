...
function [scenario_info, investment_types] = parse_all_scenarios(scenarios, run_config)
    ...
    for i = 1:n_scen
        ...
        % Early-warning precision/recall live on the run config, not on scenario structs.
        if scenario_info.early_warning(i)
            scenario_info.precision(i) = run_config.improved_ew_precision;
            scenario_info.recall(i) = run_config.improved_ew_recall;
        end
    end
end

function [annual_results, simulation_table] = event_list_simulation(simulation_table, econ_loss_model, num_sims, params)
    ...
    %                     Must include: sim_num, yr_start, calendar_years, harm_months,
    %                     is_false,
    ...
    year_dur = simulation_table.calendar_years;
    ...
    % 2. Surveillance costs (flat annual nominal fee while early warning is active)
    surveil_costs_nom = zeros(1, max_years);
    if params.improved_early_warning.active
        surveil_costs_nom = params.ew_annual_cost .* ones(1, max_years);
    end
    ...
    [sim_groups, group_ids] = findgroups(simulation_table.harm_months);
...
%   calculations for a group of events (rows) with the same duration in months.
    ...
    month_dur = group_data.harm_months;
    ...
    max_vax_rate = 0.7;
    max_uptake_per_period = cap_tot / params.P0;

    % Determine which vaccine is available each period
    response_available = (month_response_vaccine_ready <= months_matrix);

    % Initial UFV holders
    initial_ufv = double(params.universal_flu_rd.active) .* params.initial_share_ufv .* ufv_protection;

    % Phase 1: Cumulative UFV to unvaccinated (only while response not available)
    cap_for_ufv = cumsum(max_uptake_per_period .* ufv_protection .* ~response_available, 2);
    ...
    cap_for_response = cumsum(max_uptake_per_period .* response_available, 2);

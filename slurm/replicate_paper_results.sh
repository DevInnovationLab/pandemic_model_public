...
#   6    Vaccine readiness CMF + percentile table for allrisk_base (after bootstrap)
#   7    Baseline vaccine program sensitivity (chunk array + aggregation)
#   8    Advance investment sensitivity (chunk array + aggregation)
###############################################################################
...
echo "================================================================"
echo "  simulation_at_pct: allrisk_base status_quo @ 10th & 90th percentile"
...
echo "================================================================"
echo "  Vaccine readiness CMF: allrisk_base"
echo "      (after bootstrap job ${ALLRISK_BOOT} only)"
echo "================================================================"
VACCINE_READINESS_JOB="$(
  sbatch --parsable \
    --dependency="afterok:${ALLRISK_BOOT}" \
    --job-name=vacc_readiness_cmf \
    slurm/plot_vaccine_readiness_cmf.sbatch \
    output/single_runs/allrisk_base
)"
echo "Submitted: ${VACCINE_READINESS_JOB}"
echo ""

echo "================================================================"
echo "  Baseline sensitivity (status_quo_vaccine_program)"
echo "================================================================"
BASELINE_SENS_OUT="$(
  bash slurm/submit_sensitivity_workflow.sh \
    "config/sensitivity_configs/baseline_vaccine_program.yaml" \
    "${NUM_CHUNKS}" \
    response
)"
echo "${BASELINE_SENS_OUT}"
BASELINE_SENS_JOB="$(echo "${BASELINE_SENS_OUT}" | grep 'Sensitivity aggregation job submitted:' | tail -1 | awk '{print $NF}')"
echo "Submitted: ${BASELINE_SENS_JOB}"
echo ""

echo "================================================================"
echo "  Advance investment sensitivity"
echo "================================================================"
ADV_INVEST_SENS_OUT="$(
  bash slurm/submit_sensitivity_workflow.sh \
    "config/sensitivity_configs/advance_investment_programs.yaml" \
    "${NUM_CHUNKS}" \
    response
)"
echo "${ADV_INVEST_SENS_OUT}"
ADV_INVEST_SENS_JOB="$(echo "${ADV_INVEST_SENS_OUT}" | grep 'Sensitivity aggregation job submitted:' | tail -1 | awk '{print $NF}')"
echo "Submitted: ${ADV_INVEST_SENS_JOB}"
echo ""
...
echo "  vaccine readiness CMF:       ${VACCINE_READINESS_JOB}  (after ${ALLRISK_BOOT})"
echo "  Baseline sensitivity (agg):  ${BASELINE_SENS_JOB}"
echo "  Advance investment (agg):    ${ADV_INVEST_SENS_JOB}"

% Create a table summarizing results for all the levels
...
if reload_data == 1
    ...
    for l = 1:4 % loop through the level files
	    load_filename = sprintf('sim_results_level%d_sum.xlsx', l); % this file is made by gen_sim_scens.m
	    sim_results = readtable(load_filename,'Sheet','Sheet1');
    
	    sim_results = removevars(sim_results, {'sim_num'}); % remove the sim num column
        
	    avg_res_level0 = varfun(@mean, sim_results); % collapse to a rown of avgs
    
	    avg_res_level0 = remove_mean_from_col_names(avg_res_level0); % remove "mean_" from var names
        avg_res_level0 = renamevars(avg_res_level0, ...
            ["vax_costs_RD_arr", "vax_costs_time0_bn_arr", "vax_costs_in_p_ff_bn_arr"], ...
            ["vax_costs_RD_bn_arr", "vax_costs_upfront_cap_bn_arr", "vax_costs_in_p_tailoring_bn_arr"]); % TEMP
    
	    avg_res_level0.level = l; % add a level col
	    avg_res_level = [avg_res_level0(:,end) avg_res_level0(:, 1:end-1)]; % make level be first column
    
	    avg_results = [avg_results; avg_res_level]; % add to results
    end
end

if save_sum_data == 1
	outfilename = 'sim_results_CEPI_summary.xlsx';
	delete(outfilename);
...
x_cell = {};
for i = 1:length(x) % create label
    lbl = sprintf("Level %d", i);
    x_cell = [x_cell lbl];
end
...
if save_fig == 1
	outfilename = 'sim_results_CEPI_summary_figs';

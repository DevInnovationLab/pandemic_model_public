...
Inputs:  data/derived/inverted_covid_severity.yaml
...
def create_response_threshold():
    ...
    with open("./data/derived/inverted_covid_severity.yaml", "r") as f:
    ...
    for typ, val in [("intensity", covid_intensity), ("severity", covid_severity)]:
        for name, factor in [("half", 0.5), ("quarter", 0.25), ("full", 1)]:
            if factor == 1:
                outpath = f"./data/clean/response_thresholds/full_covid_{typ}.yaml"
            else:
                outpath = f"./data/clean/response_thresholds/{name}_covid_{typ}.yaml"

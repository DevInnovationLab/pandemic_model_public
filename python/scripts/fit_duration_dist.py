...
Fits a shifted lognormal distribution to pandemic duration data (years), verifies convergence
consistency, and draws parameter samples for the asymptotic MLE distribution in the unconstrained
(phi) space. CSV columns mu, sigma, and loc are in years. MATLAB ``DurationSampler`` draws
continuous years from that distribution and discretizes to integer months via round(12 * years).
...
Usage:
    python scripts/fit_mle_duration.py <fp> [--outdir PATH] [--fig-outdir PATH] [--n-samples N] [--floc F] [--seed N] [--create-fig]
"""
...
def nll_phi(phi, data, loc=0.5, transform=None):
    """Negative log-likelihood of lognormal distribution."""
...
@click.option("--max-plot-year", type=int, default=50, help='Max year to plot in the PMF.')
def fit_mle_duration(
    ...
    assert list(sample_df.columns) == ["mu", "sigma", "loc"]
    outpath = outdir / f"{outstring}.csv"
    sample_df.to_csv(outpath, index=False)
    ...
    if create_fig:
        ...
        years = np.arange(0, max_plot_year + 1, dtype=int)
        year_ticks = years[::5]
        half_width = 0.5
        ...
        ax.set_ylabel("Probability", fontsize=figstyle.axis_label_size, fontfamily=figstyle.font_family)
        ...
        t_grid = np.linspace(floc + 1e-6, max_plot_year, 300)

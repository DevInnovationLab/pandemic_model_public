"""
fit_econ_loss_models.py — Fit and visualize Poisson regression of pandemic severity against total GDP loss.
...
Outputs:
    output/econ_loss_models/econ_loss_clean.csv
    output/econ_loss_models/poisson_model_total_severity.yaml
    output/econ_loss_models/poisson_model_total_severity.pdf
    output/econ_loss_models/poisson_model_total_severity.tex
"""
...
if __name__ == "__main__":
    outdir = Path("output/econ_loss_models")
    ...
    econ_loss_clean.to_csv(outdir / "econ_loss_clean.csv", index=False)
    ...
    latex_str = (
        ...
        "\\label{tab:econ_loss_model_sev_total}\n"
        "\\begin{tabular}{lc}\n"
        "\\hline \\hline\n"
    ...
    with open(outdir / "poisson_model_total_severity.tex", "w") as f:
        f.write(latex_str)
    ...
    plt.tight_layout()
    plt.savefig(outdir / "poisson_model_total_severity.pdf", dpi=600)
    ...
    with open(outdir / "poisson_model_total_severity.yaml", "w") as f:

...
Rscript -e "renv::restore();" # Restore R environment
...
matlab -batch "${LOAD_MATPROJ}
                find_natural_covid_deaths('./config/run_configs/allrisk_base.yaml');"
...
Rscript -e "${LOAD_RENV}
            ...
            source('R/scripts/fit_arrival_risk.R');
...
python ./python/scripts/write_econ_loss_table.py

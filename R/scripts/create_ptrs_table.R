...
ptrs_table_q25 <- ptrs %>%
  left_join(pathogen_info, by = "pathogen") %>%
  select(pathogen, platform, has_prototype, q25) %>%
  mutate(has_prototype = as.numeric(has_prototype)) %>%
  rename(ptrs = q25)

ptrs_table_q75 <- ptrs %>%
  left_join(pathogen_info, by = "pathogen") %>%
  select(pathogen, platform, has_prototype, q75) %>%
  mutate(has_prototype = as.numeric(has_prototype)) %>%
  rename(ptrs = q75)
...
proto_effect_q25 <- readr::read_csv("data/clean/prototype_effect_preds_q25.csv", show_col_types = FALSE)
proto_effect_q75 <- readr::read_csv("data/clean/prototype_effect_preds_q75.csv", show_col_types = FALSE)

proto_effect_add_q25 <- ptrs_table_q25 %>%
  filter(pathogen %in% no_proto$pathogen) %>%
  left_join(proto_effect_q25 %>% select(platform, effect_mean), by = "platform") %>%
  mutate(
    ptrs = ptrs + effect_mean,
    has_prototype = 1
  ) %>%
...
proto_effect_add_q75 <- ptrs_table_q75 %>%
  filter(pathogen %in% no_proto$pathogen) %>%
  left_join(proto_effect_q75 %>% select(platform, effect_mean), by = "platform") %>%
  mutate(
    ptrs = ptrs + effect_mean,
    has_prototype = 1
  ) %>%
...
ptrs_table_final_q25 <- bind_rows(ptrs_table_q25, proto_effect_add_q25) %>%
  arrange(pathogen, platform, desc(has_prototype))

ptrs_table_final_q75 <- bind_rows(ptrs_table_q75, proto_effect_add_q75) %>%
  arrange(pathogen, platform, desc(has_prototype))

# Save the merged table as a CSV
readr::write_csv(ptrs_table_final, "data/clean/ptrs_table.csv")
readr::write_csv(ptrs_table_final_q25, "data/clean/ptrs_table_q25.csv")
readr::write_csv(ptrs_table_final_q75, "data/clean/ptrs_table_q75.csv")

...
ptrs_vf_marginal <- tibble(
  ...
  q25      = apply(mu_boot_A, 1, quantile, 0.25),
  q75      = apply(mu_boot_A, 1, quantile, 0.75),
...
proto_effect_from_A <- tibble(
  ...
  effect_q25  = sapply(delta_list, function(d) quantile(d, 0.25, na.rm = TRUE)),
  effect_q75  = sapply(delta_list, function(d) quantile(d, 0.75, na.rm = TRUE)),
...
proto_effect_q25 <- proto_effect_from_A %>%
  mutate(effect_mean = effect_q25)

proto_effect_q75 <- proto_effect_from_A %>%
  mutate(effect_mean = effect_q75)
...
readr::write_csv(proto_effect_q25, "data/clean/prototype_effect_preds_q25.csv")
readr::write_csv(proto_effect_q75, "data/clean/prototype_effect_preds_q75.csv")

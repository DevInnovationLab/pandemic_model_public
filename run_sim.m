function [vax_benefits_arr, vax_fraction_cum_end, PV_factor, in_pandemic_cap_costs_PV, in_pandemic_marg_costs_m_PV, in_pandemic_marg_costs_o_PV] = run_sim(params, yr_start, is_false, ...
	pandemic_natural_dur, state, intensity, x_m, x_o, z_m, z_o)

	% run simulation for a pandemic of significant size

		if is_false == 1
            tot_months = params.tau_A; % if is false pos, then will find out by end of tau_A (~100 day period)
        else
